...
/**
 * @typedef {foundry.data.types.StringFieldOptions & _LocalDocumentFieldOptions} LocalDocumentFieldOptions
 */

/**
 * A mirror of ForeignDocumentField that references a Document embedded within this Document.
 */
export default class LocalDocumentField
  extends foundry.data.fields.DocumentIdField
{
  /**
   * @param {typeof foundry.abstract.Document} model - The local DataModel class definition which this field should link to.
  ...
  _findCollection(model, collection) {
    ...
    try {
      ...
      return (
        ...
        this._findCollection(model.parent, collection)
      );
    }
  }
  ...
  initialize(value, model, options = {}) {
    if (this.idOnly)
      return this.options.fallback || foundry.data.validators.isValidId(value)
        ? value
        : null;
    const collection = this._findCollection(
      model,
      this.model.metadata.collection,
    );
    return () => {
