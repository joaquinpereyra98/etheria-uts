...
/**
 * Data model for standard Etheria chat messages.
 * Handles modular "parts" that can be rendered dynamically.
 */
export default class EtheriaStandardData extends TypeDataModel {
  /** @override */
  static defineSchema() {
    return {
      parts: new TypedObjectField(new TypedSchemaField(MessagePart.TYPES), {
  ...
  /** @type {boolean} */
  get isRoll() {
    return Object.values(this.parts).some((p) => p.isRoll);
  }

  /** @type {boolean} */
  get visible() {
    return Object.values(this.parts).some((p) => p.visible);
  }

  /**
   * Total number of rolls currently visible to the user.
   * @type {number}
   */
  get _rollCount() {
    return Object.values(this.parts).reduce(
      (acc, p) => acc + (p.visible ? p.rolls.length : 0),
      0,
    );
  }
  ...
  _onCreate(data, options, userId) {
    ...
    for (const part of Object.values(this.parts))
      part._onCreate(data, options, userId);
  }
  ...
  async renderHTML(options = {}) {
    const headerTemplate = `${TEMPLATE_PATH}/chat-messages/header.hbs`;
    const context = {
      ...options,
    ...
    for (const [id, part] of Object.entries(this.parts)) {
      ...
      const htmlString = await handlebars.renderTemplate(
        part.constructor.TEMPLATE,
        context,
      );
      const html = foundry.utils.parseHTML(`
