...
export default function simplifyRollFormula(
  formula,
  { preserveFlavor = false, deterministic = false } = {},
) {
  ...
  if (deterministic) {
    ...
    for (let i = roll.terms.length - 1; i >= 0; ) {
      ...
      if (term instanceof ParentheticalTerm) {
        paren = simplifyRollFormula(term.term, {
          preserveFlavor,
          deterministic,
        });
      }
...
function _simplifyNumericTerms(terms) {
  ...
  if (unannotated.length) {
    ...
    simplified.push(
      new OperatorTerm({ operator: staticBonus < 0 ? "-" : "+" }),
    );
...
function _simplifyDiceTerms(terms) {
  ...
  const diceQuantities = unannotated.reduce((obj, curr, i) => {
    ...
    obj[key] ??= {};
    if (curr._number instanceof Roll && curr._number.isDeterministic)
      curr._number.evaluateSync();
  ...
  const simplified = Object.entries(diceQuantities).flatMap(
    ([key, { number, modifiers }]) => [
      new OperatorTerm({ operator: key.charAt(0) }),
      key.slice(1) === "c"
        ? new Coin({ number: number })
        : new Die({
            number,
            faces: parseInt(key.slice(1)),
...
function _groupTermsByType(terms) {
  ...
  return terms.reduce((obj, term, i) => {
    ...
    else if (term instanceof FunctionTerm && term.isDeterministic)
      type = NumericTerm;
